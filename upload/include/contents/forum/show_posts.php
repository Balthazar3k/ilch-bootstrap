...
if (($aktTopicRow['stat'] == 1 AND $forum_rights['reply'] == TRUE) OR ($_SESSION['authright'] <= '-7' OR $forum_rights['mods'] == TRUE)) {
  $antworten = '<a class="btn btn-primary btn-xs" href="index.php?forum-newpost-'.$tid.'">'.$lang['answer'].'</a>';
}
...
while($row = db_fetch_assoc($erg)) {
	...
	# define some vars.
	$row['sig'] = ( empty($row['sig']) ? '' : '<br><br><div class="well well-sm">'.bbcode($row['sig']).'</div>' );
	$row['TID'] = $tid;
	$row['class'] = $class;
    $times = $row['time'];
$diff = time() - $row['time']; 
...
$wochentag = strftime("%A", $times); 
	...
	if (file_exists($row['avatar'])) { $row['avatar'] = '<br /><br /><img class="avatar_height" src="'.$row['avatar'].'" alt="User Pic" border="0" /><br />'; }
	elseif ($allgAr['forum_default_avatar']) { $row['avatar'] = '<br /><br /><img class="avatar_height" src="include/images/avatars/'.$ges_ar[$row['geschlecht']].'.jpg" alt="User Pic" border="0" /><br />'; }
 	else { $row['avatar'] = ''; }
	...
	if ($forum_rights['mods'] == TRUE AND $i>1) {
	  $row['delete'] = '<li class="divider"></li><li><a href="index.php?forum-delpost-'.$tid.'-'.$row['id'].'">'.$lang['delete'].'</a></li>';
	}
	if ( $forum_rights['reply'] == TRUE AND loggedin() ) {
	  $row['change'] = '<li><a href="index.php?forum-editpost-'.$tid.'-'.$row['id'].'">'.$lang['change'].'</a></li>';
	}
	$row['posts']  = ($row['posts']?'<br><span class="label label-default">Posts: '.$row['posts']:'').'</span><br>';
